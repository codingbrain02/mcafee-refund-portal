...
## Verified eligible-order refund RPC

`submit_eligible_order_refund(p_order_id uuid, p_refund_reason text)` creates a customer refund from a staff-maintained eligible order.

- Authentication: verified Customer account.
- Ownership: the order email must match the active account email.
- Locked values: order number, product, purchase date, amount, and method come from the order record.
- Reference: generated by PostgreSQL.
- Duplicate protection: one refund per eligible order.
