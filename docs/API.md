...
`submit_customer_refund_request(p_order_number text, p_product_name text, p_refund_reason text)` creates a Submitted customer refund request without requiring a staff-created order record.

- Authentication: verified Customer account.
- Customer input: order number, antivirus product, reason, and optional supporting documents.
- Protected values: amount, purchase date, and refund method remain pending for staff verification.
