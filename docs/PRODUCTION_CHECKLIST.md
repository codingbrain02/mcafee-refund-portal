...
- [ ] Monitoring remains disabled and the portal stays healthy when Sentry DSNs are blank.
- [ ] Preview health reports monitoring as configured after DSNs are added.
- [ ] Controlled preview frontend and API errors appear with release and environment tags.
- [ ] Captured events contain no customer PII, credentials, request bodies, or banking data.
- [ ] Source maps resolve production stack frames after build credentials are configured.
