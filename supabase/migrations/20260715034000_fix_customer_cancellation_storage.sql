...
begin
  ...
  where request.id = p_refund_request_id
    and (
      ...
      or (actor_email is not null and lower(customer.email) = lower(actor_email))
    );
  ...
  delete from public.refund_requests where id = owned_request.id;

  delete from public.customers customer
  where customer.id = owned_request.customer_id
    and not exists (
      select 1 from public.refund_requests request where request.customer_id = customer.id
    );
